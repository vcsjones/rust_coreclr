...
impl Drop for CoreCLR {
    fn drop(&mut self) {
        ...
        let shutdown_func: libloading::Symbol<CoreCLRShutdown> = unsafe { self.library.get(b"coreclr_shutdown").unwrap() };
...
impl CoreCLR {
    pub fn create_delegate<'clr, T: 'clr>(&'clr self, assembly_name : &str, type_name : &str, method_name : &str) -> Result<ManagedDelegate<'clr, T>> {
        let create_delegate: libloading::Symbol<CoreCLRCreateDelegate> = unsafe { self.library.get(b"coreclr_create_delegate")? };
    ...
    pub fn new(library_path : &str, exe_path : &str, app_domain_friendly_name : &str, properties : &HashMap<&str, &str>) -> Result<CoreCLR> {
        ...
        let mut domain_id = 0;
        let result = {
            let initializer: libloading::Symbol<CoreCLRInitialize> = unsafe { library.get(b"coreclr_initialize")? };
            initializer(
                exe_path_ptr.as_ptr(),
